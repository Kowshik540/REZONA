...
function TailorModal({ job, skills, resumeId, onClose }) {
  ...
  async function tailor() {
    ...
    try {
      // Try server first (works if API credits are added later)
      let data = null;
      try {
        ...
        if (res.data?.professionalSummary) data = res.data;
      } catch (_) {
        // Server unavailable or no credits — use local engine below
      }

      // Always fall back to local engine if server didn\'t return a good result
      if (!data) data = localTailor(job, skills);
...
<div class="page">
  ...
  ${skillsHTML ? `
  <div class="section">
    <div class="section-title">Key Skills to Highlight</div>
    <div class="skills-wrap">${skillsHTML}</div>
  </div>` : ''}

  ${bulletsHTML ? `
  <div class="section">
    <div class="section-title">Tailored Experience Bullets</div>
    ${bulletsHTML}
  </div>` : ''}

  ${r.changes?.length ? `
  <div class="section">
    <div class="section-title">Changes Made by AI</div>
    ${r.changes.map(c => `<div class="change-item"><span class="check">✓</span>${c}</div>`).join('')}
  </div>` : ''}

  <div class="footer">
    Generated by Rezona • Tailored for "${job.title}" at ${job.company} • Update your personal details before submitting
  </div>
              ...
              {activeTab === 'preview' && (
                <div>
                  ...
                  <div style={m.actionRow}>
                    ...
                    <button style={m.downloadBtn} onClick={() => downloadPDF(result)}>
                      📄 Download PDF
                    </button>
                  ...
                  <p style={m.hintText}>
                    Click "Download PDF" → your browser opens a print dialog → choose "Save as PDF".
                  </p>
                </div>
              )}
